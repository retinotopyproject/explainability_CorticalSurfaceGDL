...
train_dataset = Retinotopy(path, 'Train', transform=T.Cartesian(max_value=norm_value),
                           ...
                           prediction='eccentricity', myelination=False,
                           hemisphere='Right') # Change to Left for the LH

'''
Check that the training dataset is using the new curvature data 
...
for i in range(5):
    ...
    torch.save(model.state_dict(),
               osp.join(osp.dirname(osp.realpath(__file__)), 'output',
                        'deepRetinotopy_ECC_RH_model' + str(i+1) + '.pt')) # Rename if LH
