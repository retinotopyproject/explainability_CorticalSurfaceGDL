...
for the ECC LH/RH Test set, which are used in 
...
The code requires that several other files are generated first:
    ...
    - Manuscript/plots/output/{dataset_filename}AverageEccentricityMap_{L or R}H.npz (generated 
    by running Manuscript/plots/{dataset_filename}SuppFigure3_ECCaverage_plot.py for the 
    corresponding hemisphere for the Test set)
    - The ECC Test set results for the best performing model. This can be used
    for HCP, or NYU non-finetuned models saved in Models/generalizability. 
    Copy the model '{dataset_filename}testset-intactData_ECC_{L or R}H_model*.pt' to 
    Manuscript/testset_results, then rename the model to 
    '{dataset_filename}testset-pred_deepRetinotopy_ECC_{L or R}H.pt'.
...
def ECC_difference(model, hemisphere='Left', dataset='HCP'):
    ...
    Output: .npz files in ./../output named:
        '{dataset_filename}ErrorPerParticipant_ECC_{L or R}H_WangParcels_' + str(model) + '_1-8.npz'
        '{dataset_filename}ErrorPerParticipant_ECC_{L or R}H_EarlyVisualCortex_' + str(model) +
        '_1-8.npz'
        '{dataset_filename}ErrorPerParticipant_ECC_{L or R}H_dorsalV1-3_' + str(model) + '_1-8.npz'
    ...
    # Average map
    ecc_average = \
        np.load(f'./../plots/output/{dataset_filename}AverageEccentricityMap_' +
    ...
    for k in range(len(visual_areas)):
        ...
        for m in range(len(models)):
            predictions = torch.load(
                ...
                f'ECC_{hemi_filename}H.pt', map_location='cpu')
    ...
    np.savez(f'./output/{dataset_filename}ErrorPerParticipant_ECC_{hemi_filename}H_' +
...
ECC_difference('average', 'Left', 'HCP')
ECC_difference('deepRetinotopy', 'Left', 'HCP')
# ECC_difference('average', 'Right', 'HCP')
# ECC_difference('deepRetinotopy', 'Right', 'HCP')
# ECC_difference('average', 'Left', 'NYU')
# ECC_difference('deepRetinotopy', 'Left', 'NYU')
# ECC_difference('average', 'Right', 'NYU')
# ECC_difference('deepRetinotopy', 'Right', 'NYU')
