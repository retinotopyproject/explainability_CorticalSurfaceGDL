...
The code requires that several other files are generated first:
    ...
    - Manuscript/plots/output/{train_mean_filename}AverageEccentricityMap_{L or R}H.npz 
    (generated by running Manuscript/plots/{HCP or NYU}SuppFigure3_ECCaverage_plot.py 
    for the corresponding hemisphere and model)
...
def ECC_difference(model, hemisphere='Left', selected_model=1, dataset='HCP', 
    ...
    # Average map
    ecc_average = \
        np.load(f'./../plots/output/{train_mean_filename}AverageEccentricityMap_' +
                f'{hemi_filename}H.npz')['list']

    for k in range(len(visual_areas)):
        mean_delta = []
        predictions = torch.load(osp.join('./../..', 'Models', 'generalizability', 
        testset_results_dir, f'{dataset_filename}-intactData_ECC_' + 
    ...
    np.savez(f'./output/{output_filename}ErrorPerParticipant_ECC_{hemi_filename}H_' +
...
#     ECC_difference('average', hemi, 5, 'HCP')
#     ECC_difference('deepRetinotopy', hemi, 5, 'HCP')

# # Generates files for NYU models (no finetuning)
# for hemi in ['Left', 'Right']:
#     ECC_difference('average', hemi, 5, 'NYU')
#     ECC_difference('deepRetinotopy', hemi, 5, 'NYU')
...
#             ECC_difference('average', hemi, 5, 'NYU', n_subj, epochs)
#             ECC_difference('deepRetinotopy', hemi, 5, 'NYU', n_subj, epochs)
