...
for ECC LH/RH, which are used in Figure7b_DeltaTheta_ModelvsAverageMap.py.
...
The code requires that several other files are generated first:
    ...
    - Manuscript/plots/output/AverageEccentricityMap_{L or R}H.npz (generated 
    by running Manuscript/plots/SuppFigure3_ECCaverage_plot.py for the 
    ...
    Copy the model 'testset-intactData_ECC_{L or R}H_model*.pt' to 
    Manuscript/testset_results, then rename the model to 
    'testset-pred_deepRetinotopy_ECC_{L or R}H.pt'.
"""
...
def ECC_difference(model):
    ...
    Output: .npz files in ./../output named:
        'ErrorPerParticipant_ECC_{L or R}H_WangParcels_' + str(model) + '_1-8.npz'
        'ErrorPerParticipant_ECC_{L or R}H_EarlyVisualCortex_' + str(model) +
        '_1-8.npz'
        'ErrorPerParticipant_ECC_{L or R}H_dorsalV1-3_' + str(model) + '_1-8.npz'
    ...
    # Average map
    ecc_average = \
        np.load('./../plots/output/AverageEccentricityMap_' +
    ...
    np.savez(f'./output/ErrorPerParticipant_ECC_{HEMI_FILENAME}H_' +
