...
for index in range(0, len(subjects)):
    ...
    old_data = nib.load(osp.join(path, f'sub-wlsubj{subjects[index]}', 
        f'sub-wlsubj{subjects[index]}.curv.lh.32k_fs_LR.func.gii'))
    old_data = torch.tensor(np.reshape(old_data.agg_data()
...
def plot_curvatures(subject_index, scaling):
    ...
    background = np.array(np.reshape(curv_old[subject_index][
        0:number_hemi_nodes], (-1)))
    ...
    view = plotting.view_surf(
        ...
        black_bg=False, symmetric_cmap=False, title='Subject ' +
        f'{subjects[subject_index]}: NYU curvature data (Left hemisphere)')
    # View in browser
    ...
    #     {subjects[subject_index]}: Prev curvature data (Left hemisphere)')
    # # View in browser
