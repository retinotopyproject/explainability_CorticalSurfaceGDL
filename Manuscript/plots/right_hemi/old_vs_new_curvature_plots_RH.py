...
for index in range(0, len(subjects)):
    ...
    new_data = nib.load(osp.join(path, f'fs-curvature/{subjects[index]}/', 
        f'{subjects[index]}.R.curvature.32k_fs_LR.shape.gii'))
...
def plot_curvatures(subject_index, scaling):
    ...
    if scaling:
        ...
        curv_old_masked[final_mask_R == 1] = np.divide(np.power(5, 
            np.reshape(np.asarray(curv_old[subject_index][final_mask_R == 1]), 
            (-1, 1)) + 10), 100000)
        curv_old_masked[final_mask_R != 1] = 0

        # New curvature - Masking w/ scaling applied
        curv_new_masked[final_mask_R == 1] = np.divide(np.power(5, 
            np.reshape(np.asarray(curv_new[subject_index][final_mask_R == 1]), 
            (-1, 1))+ 10), 100000)
        curv_new_masked[final_mask_R != 1] = 0

        scaling_dir = 'rescaled'
    else:
        # No re-scaling applied
        # Masking - only translating values up
        curv_old_masked[final_mask_R == 1] = (np.reshape(np.asarray(
            curv_old[subject_index][final_mask_R == 1]), (-1, 1)) + 1.5)
        curv_old_masked[final_mask_R != 1] = 0

        # New curvature - Masking - only translating values up
        curv_new_masked[final_mask_R == 1] = (np.reshape(np.asarray(
            curv_new[subject_index][final_mask_R == 1]), (-1, 1)) + 1.5)
        curv_new_masked[final_mask_R != 1] = 0
    ...
    view = plotting.view_surf(
        surf_mesh=osp.join(osp.dirname(osp.realpath(__file__)), '../../..',
                    ...
                    f'/S1200_7T_Retinotopy181.R.sphere.32k_fs_LR.surf.gii'),
        ...
        {subjects[subject_index]}: Old curvature data (Right hemisphere)')
    # View in browser
    view.open_in_browser()
    # Save as html file
    # view.save_as_html(f'D:\\Retinotopy Project\\surf_images\\\
    #     old_vs_new_curvature\\{scaling_dir}\\\
    #     {subjects[subject_index]}_RH_oldcurv')
    ...
    view_new = plotting.view_surf(
        ...
        0:number_hemi_nodes], (-1)), threshold=threshold, cmap='plasma', 
        black_bg=False, symmetric_cmap=False, title=f'Subject \
        {subjects[subject_index]}: New curvature data (Right hemisphere)')
    # View in browser
    view_new.open_in_browser()
    # Save as html file
    # view_new.save_as_html(f'D:\\Retinotopy Project\\surf_images\\\
    #     old_vs_new_curvature\\{scaling_dir}\\\
    #     {subjects[subject_index]}_RH_newcurv')
